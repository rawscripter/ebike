  $(document).on('click',"tr.info_row",function (event) {
            ...
            var ownerData = $(this).data('info');

            // setting for update and delete with ajax
            $('.modal-image').attr('src',ownerData.image);
            $('.deleteable_id').val(ownerData.id);
            $('.updateable_id').val(ownerData.id);

            $.each(ownerData, function (key, value) {
